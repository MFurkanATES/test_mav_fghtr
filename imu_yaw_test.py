...
def publish_yaw():
    ...
    data = TwistStamped()
    print(data)
    euler_new = (X,Y,math.radians((Z_degree + 100)))
    
    qua = tf.transformations.quaternion_from_euler(euler_new[0],euler_new[1],euler_new[2])

    data.twist.angular.x = 0
    data.twist.angular.y = 0
    data.twist.angular.z = math.radians(-100)
